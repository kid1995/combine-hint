...
/**
 * Checks out the given branch and performs a series of cleanup operations to ensure a clean workspace.
 *
 * @param name The name of the branch to check out.
 * @param notificationConfig (Optional) A map for configuring notifications.
 *
 * @return A Map<String, String> containing the SCM variables that Jenkins determined during the checkout.
 *         This map includes crucial keys like:
 *         - 'GIT_COMMIT': The hash of the current commit that was checked out.
 *         - 'GIT_PREVIOUS_SUCCESSFUL_COMMIT': The hash of the last commit from a successful build on this branch.
 *         - 'GIT_BRANCH': The name of the branch.
 *         - 'GIT_URL': The repository URL.
 *         The caller is responsible for handling this map. Returns an empty map if the checkout fails to produce variables.
 */
public Map<String, String> checkoutBranch(String name, Map notificationConfig = [:]) {
