...
/**
 * Executes a series of commands within a persistent Docker container that has the Jenkins workspace mounted.
 *
 * This function can be used to run command-line tools (like helm, kustomize, etc.) that need
 * access to the project's source files. It handles all the complexity of starting a container
 * on the remote Docker build farm, copying the workspace, executing commands, and ensuring cleanup.
 *
 * @param serviceGroup The service group, used for the --config parameter to isolate docker logins.
 * @param imageName The full name of the Docker image to use (e.g., 'my-registry/my-image:tag').
 * @param runFn A closure that receives one or two arguments:
 *              1. `runCommand` (Function): A helper to execute shell commands inside the container.
 *              2. `containerId` (String, optional): The full ID of the running container for advanced use cases.
 *
 * Example (Simple Use Case):
 * <pre><code>
 * si_docker.withContainer('my-group', 'my-image:tag') { runCmd ->
 *     def version = runCmd('helm version')
 ...
 * <pre><code>
 * si_docker.withContainer('my-group', 'my-image:tag') { runCmd, containerId ->
 *     echo "Building kustomize overlay..."
 *     def kustomizeOutput = runCmd('kustomize build --enable-helm envs')
 *     writeFile file: 'result.yaml', text: kustomizeOutput
 *     echo "File 'result.yaml' created successfully in the Jenkins workspace."
 *
 *     sh "docker exec ${containerId} sh -lc 'ps aux'"
 * }
 * </code></pre>
 */
public void withContainer(String serviceGroup, String imageName, Closure runFn) {
    def cid = '' // Will hold the Container ID

    try {
        loginToSDADockerRegistries(serviceGroup)

        // Start the persistent container in the background.
        echo "[Docker] Starting persistent container from image: ${imageName}"
        cid = sh(
                script: """
                    docker --config ${serviceGroup} ${DOCKER_TLS_PARAMETER} run -d \
                      -w /workspace \
                      ${imageName} \
                      sh -lc 'mkdir -p /workspace; sleep infinity'
                  """,
                returnStdout: true
        ).trim()
        echo "[Docker] Container started with ID: ${cid}"

        // Copy the entire current Jenkins workspace into the container's /workspace directory.
        echo "[Docker] Copying current workspace into container..."
        sh "docker --config ${serviceGroup} ${DOCKER_TLS_PARAMETER} cp . ${cid}:/workspace"

        // Log the content of the copied workspace for debugging purposes.
        sh "docker --config ${serviceGroup} ${DOCKER_TLS_PARAMETER} exec ${cid} sh -lc 'echo \"[Docker] Content of /workspace inside container:\"; ls -la /workspace || true'"

        // Define the helper function that will be passed to the user.
        def runCommand = { commandStr ->
            echo "[Docker] Executing in container: ${commandStr}"
            return sh(
                    script: "docker --config ${serviceGroup} ${DOCKER_TLS_PARAMETER} exec ${cid} sh -lc 'cd /workspace && ${commandStr}'",
                    returnStdout: true
            ).trim()
        }

        // Execute the user's pipeline logic, passing the helper function and the container ID.
        echo "[Docker] Handing over to user-defined closure..."
        runFn(runCommand, cid)
        echo "[Docker] User-defined closure finished."
    } finally {
        // Cleanup: Always remove the container after the block has finished.
        if (cid) {
            echo "[Docker] Cleaning up container ID: ${cid}"
            try {
                sh "docker --config ${serviceGroup} ${DOCKER_TLS_PARAMETER} rm -f ${cid} || true"
            } catch (ignore) {
                // To avoid failing the entire build just for a cleanup error, we log it as a warning.
                echo "[Docker] Warning: Failed to clean up container ${cid}, but continuing."
            }
        }
    }
}
